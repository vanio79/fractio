...
type
  CreateOptions* = object
    ...
    # Compaction strategy
    compactionStrategy*: object  # Placeholder for compaction strategy
...
proc defaultCreateOptions*(): CreateOptions =
  CreateOptions(
    ...
    indexBlockPartitioningPolicy: PartitioningPolicy(partitions: @[false, false, false, true]),
    filterBlockPartitioningPolicy: PartitioningPolicy(partitions: @[false, false, false, true]),
    expectPointReadHits: false,
    filterPolicy: FilterPolicy(entries: @[
      FilterPolicyEntry(kind: fpeBloom, bloomPolicy: BloomConstructionPolicy(kind: bcpFalsePositiveRate, falsePositiveRate: 0.0001)),
      FilterPolicyEntry(kind: fpeBloom, bloomPolicy: BloomConstructionPolicy(kind: bcpBitsPerKey, bitsPerKey: 10.0))
    ]),
    levelCount: 7,
...
proc fromKvs*(keyspaceId: InternalKeyspaceId, metaKeyspace: MetaKeyspace): Option[CreateOptions] =
...
# Builder methods
proc withKvSeparation*(options: CreateOptions, opts: Option[KvSeparationOptions]): CreateOptions =
  var result = options
  result.kvSeparationOpts = opts
  return result

proc dataBlockRestartIntervalPolicy*(options: CreateOptions, policy: RestartIntervalPolicy): CreateOptions =
  var result = options
  result.dataBlockRestartIntervalPolicy = policy
  return result

proc filterBlockPinningPolicy*(options: CreateOptions, policy: PinningPolicy): CreateOptions =
  var result = options
  result.filterBlockPinningPolicy = policy
  return result

proc indexBlockPinningPolicy*(options: CreateOptions, policy: PinningPolicy): CreateOptions =
  var result = options
  result.indexBlockPinningPolicy = policy
  return result

proc filterBlockPartitioningPolicy*(options: CreateOptions, policy: PartitioningPolicy): CreateOptions =
  var result = options
  result.filterBlockPartitioningPolicy = policy
  return result

proc indexBlockPartitioningPolicy*(options: CreateOptions, policy: PartitioningPolicy): CreateOptions =
  var result = options
  result.indexBlockPartitioningPolicy = policy
  return result

proc dataBlockHashRatioPolicy*(options: CreateOptions, policy: HashRatioPolicy): CreateOptions =
  var result = options
  result.dataBlockHashRatioPolicy = policy
  return result

proc filterPolicy*(options: CreateOptions, policy: FilterPolicy): CreateOptions =
  var result = options
...
proc dataBlockCompressionPolicy*(options: CreateOptions, policy: CompressionPolicy): CreateOptions =
  var result = options
  result.dataBlockCompressionPolicy = policy
  return result

proc indexBlockCompressionPolicy*(options: CreateOptions, policy: CompressionPolicy): CreateOptions =
  var result = options
...
proc dataBlockSizePolicy*(options: CreateOptions, policy: BlockSizePolicy): CreateOptions =
  var result = options
