# Recovery Test 4 - Step 2: Parent verifies persistent data
# Run after 04_persist_child.nim
...
let testPath = "tmp/raft_recovery_persist/"
...
if not entry.isSome or entry.get.data != "persistent data":
  echo "FAIL: persistent data not found or wrong data"
  quit(1)

raftNode.shutdown()
removeDir(testPath)
echo "OK: Node state persists across restart verified"
