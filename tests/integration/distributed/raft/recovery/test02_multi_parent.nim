# Recovery Test 2 - Step 2: Parent verifies multiple log entries recovery
# Run after 02_multi_child.nim
...
if raftNode.wsLogStore.nextIndex != 4:
  echo "FAIL: Expected nextIndex=4, got ", raftNode.wsLogStore.nextIndex
  quit(1)

let entry1 = raftNode.wsLogStore.getEntry(1)
let entry2 = raftNode.wsLogStore.getEntry(2)
let entry3 = raftNode.wsLogStore.getEntry(3)

if not entry1.isSome or entry1.get.data != "entry1":
  echo "FAIL: entry1 not found or wrong data"
  quit(1)

if not entry2.isSome or entry2.get.data != "entry2":
  echo "FAIL: entry2 not found or wrong data"
  quit(1)

if not entry3.isSome or entry3.get.data != "entry3":
  echo "FAIL: entry3 not found or wrong data"
  quit(1)

raftNode.shutdown()
removeDir(testPath)
echo "OK: Recovery with multiple log entries verified"
