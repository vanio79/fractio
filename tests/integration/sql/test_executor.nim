...
suite "SQL Executor — SHOW statements":
  ...
  test "SHOW SCHEMAS empty":
    ...
    # 'sys' is an implicit schema on all databases
    check res.rows.len == 1
    check res.rows[0][0] == "sys"
  ...
  test "SHOW SCHEMAS after creating some":
    ...
    let res = client.exec("SHOW SCHEMAS", database = "mydb")
    check res.kind == erkRows
    # Note: CREATE DATABASE auto-creates "public" schema, plus implicit "sys"
    check res.rows.len == 4
    var names: seq[string]
    for row in res.rows:
      names.add(row[0])
    check "sys" in names      # implicit
