...
import fractio/protocol/mvcc_store as mvccStore
...
suite "MVCC Version Key Detection (mvcc_store)":
  ...
  test "isVersionKey true - basic version key":
    ...
    for i in 0..<8:
      key.add(chr(i))
    check mvccStore.isVersionKey(key) == true
  ...
  test "isVersionKey false - regular key":
    check mvccStore.isVersionKey("user_key") == false

  test "isVersionKey false - too short":
    check mvccStore.isVersionKey("abc") == false
  ...
  test "isVersionKey false - intent key (\\x00\\x01 suffix)":
    ...
    for i in 0..<16:
      key.add(chr(i))
    check mvccStore.isVersionKey(key) == false
...
suite "MVCC Intent Key Detection (mvcc_store)":
  ...
  test "isIntentKeyMvcc true - basic intent key":
    ...
    for i in 0..<16:
      key.add(chr(i))
    check mvccStore.isIntentKeyMvcc(key) == true
  ...
  test "isIntentKeyMvcc false - regular key":
    check mvccStore.isIntentKeyMvcc("user_key") == false

  test "isIntentKeyMvcc false - too short":
    check mvccStore.isIntentKeyMvcc("abc") == false
  ...
  test "isIntentKeyMvcc false - version key (\\x00\\x00 suffix)":
    ...
    for i in 0..<8:
      key.add(chr(i))
    check mvccStore.isIntentKeyMvcc(key) == false
  ...
  test "isIntentKeyMvcc does not match protocol intent keys":
    ...
    check mvccStore.isIntentKeyMvcc(protoKey) == false

suite "MVCC Key Filtering Consistency":
  # Verify that mvcc_store's isVersionKey/isIntentKeyMvcc match constants from mvccTypes
