...
suite "Allocator":
  ...
  test "should rebalance under-replicated":
    ...
    let decisions = alloc.shouldRebalance(RangeID(1), replicas, NodeID(1))
...
suite "Allocation Decision":
  test "create add replica decision":
    let decision = newAddReplicaDecision(RangeID(1), NodeID(2), 10, "test")
  ...
  test "create remove replica decision":
    let decision = newRemoveReplicaDecision(RangeID(1), NodeID(2), 5, "test")
  ...
  test "create transfer lease decision":
    let decision = newTransferLeaseDecision(RangeID(1), NodeID(1), NodeID(2), 3, "test")
...
suite "Rebalance Queue":
  ...
  test "priority ordering":
    ...
    let low = newAddReplicaDecision(RangeID(2), NodeID(1), 1, "low")
    let high = newAddReplicaDecision(RangeID(1), NodeID(2), 10, "high")
...
suite "Rebalance Scheduler":
  ...
  test "process batch":
    ...
    for i in 1..5:
      let decision = newAddReplicaDecision(RangeID(i), NodeID(2), 10, "test")
...
suite "Merge Executor":
  ...
  test "create merged descriptor":
    ...
    let right = newRangeDescriptor(
      ...
      @[byte(50)],
      @[byte(100)],
      @[newReplicaDescriptor(NodeID(1), ReplicaID(2))]
    )
