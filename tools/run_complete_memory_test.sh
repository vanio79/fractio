#!/bin/bash
# Complete end-to-end memory test: setup + Phase A INSERT under 250MB limit enforcement
# This script properly sequences smoke_setup followed by Phase A INSERT with monitoring
...
echo "=========================================="
echo "Complete Memory-Enforced Smoke Test"
...
# Run smoke_setup first (creates space and table)
echo ""
echo "=== Running smoke_setup ==="
timeout 180 bin/smoke_setup > /tmp/smoke_setup.log 2>&1 &
SETUP_PID=$!
...
if [ $setup_exit -eq 0 ]; then
    echo "✓ smoke_setup completed successfully"
else
    echo "✗ smoke_setup failed with exit code $setup_exit"
    tail -30 /tmp/smoke_setup.log
    pkill -9 -f "bin/fractio" 2>/dev/null || true
...
#!/bin/bash
# Wrapper to run Phase A and report memory periodically — NO TIMEOUT, run until complete
...
bin/smoke_1m_inserts_deletes >> "$LOGFILE" 2>&1 &
TEST_PID=$!
...
# Run the Phase A test with monitoring (no timeout — will run until complete)
/tmp/run_phase_a.sh &
TEST_PID=$!

echo "Phase A test started (PID $TEST_PID)"
echo "Monitor log: /tmp/phase_a_mem_monitor.log"
echo "Waiting for test to complete (no timeout, unlimited duration)..."

# Wait indefinitely for the test to finish
wait $TEST_PID
test_exit=$?
...
PHASE_A_END=$(date +%s)
PHASE_A_ELAPSED=$((PHASE_A_END - PHASE_A_START))
MINUTES=$((PHASE_A_ELAPSED / 60))
SECONDS=$((PHASE_A_ELAPSED % 60))

echo ""
echo "=========================================="
echo "Phase A Test Complete (exit code: $test_exit)"
echo "Duration: ${MINUTES}m ${SECONDS}s (${PHASE_A_ELAPSED}s total)"
echo "=========================================="
...
echo ""
echo "=== Memory Monitor Log (Last 150 lines) ==="
if [ -f /tmp/phase_a_mem_monitor.log ]; then
    tail -150 /tmp/phase_a_mem_monitor.log
else
    echo "No monitor log found"
fi

# Show setup log summary
echo ""
echo "=== smoke_setup Log (Last 30 lines) ==="
if [ -f /tmp/smoke_setup.log ]; then
    tail -30 /tmp/smoke_setup.log
else
